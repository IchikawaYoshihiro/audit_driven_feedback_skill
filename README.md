...
| **knowledge-audit** | AI への知識は**正しい層にあるか** | 5層アーキテクチャに沿って知識を昇格・降格・棚卸し。望ましい状態へのリコンシリエーションを AI が実行できる手順として形式知化した Knowledge Reconciler |

`audit-driven-feedback` が「どの監査を持つべきか」を決め、`assurance-audit` が「ある防御が
本当にテストされ・そのテストが演技でないか」を採点し、`knowledge-audit` が「AI へ渡す知識が
正しい層に置かれ SSoT が保たれているか」を監査・整合します。以下はまず `audit-driven-feedback`
の説明で、`assurance-audit` と `knowledge-audit` は末尾の専用節を参照してください。
...
Claude Memory が使えるようになって全部入れたところ、数週間で運用が破綻しました。問題は Memory
ではなく、**知識のライフサイクルを設計していなかった**ことです。`knowledge-audit` は、AI へ渡す
知識を 5 層アーキテクチャに沿って仕分け直し、**望ましい状態へ継続的にリコンシリエーション**します。

5 層の役割:

| 層 | 場所 | 役割 |
|---|---|---|
| Policy | `~/.claude/CLAUDE.md` | 何を守るか（原則・禁止・昇格基準） |
| Knowledge | `~/.claude/knowledge/*.md` | なぜそうするか（Fact/Rules/Why） |
| Skill | `~/.claude/commands/*.md` | どうやるか（手順・チェックリスト） |
| Project | `projects/<name>/memory/` | このプロジェクト固有の判断 |
| Session | 現在の会話 | 今回限りの文脈 |

スキルがすること:

1. **Candidate Inventory** — AGENTS.md / memory/ を走査して昇格・降格候補を一覧化
2. **Promotion Gate** — 5 項目チェック（2PJ以上 / 半年有効 / PJ非依存 / Skill化不要 / Why説明可）
3. **Knowledge ファイル作成** — Fact/Rules/Why 形式で `~/.claude/knowledge/` に格納
4. **Trigger Table 更新** — CLAUDE.md に「いつ読むか」を登録
5. **Policy Audit** — CLAUDE.md に "How" が混入していないかチェック・Skill 層へ降格
6. **Work Summary** — 昇格 / 降格 / 見送り / 変更なし の 4 セクションで報告

```
knowledge-audit/
├── SKILL.md                      # 7ステップのワークフロー（エントリポイント）
└── evals/
    └── evals.json                # 3ケース（candidate scan / demotion check / full audit）
```

インストールは他のスキルと同じ要領で、`knowledge-audit/` ディレクトリを
`~/.claude/skills/` に置きます。「knowledge を整理したい」「CLAUDE.md を棚卸ししたい」
「AGENTS.md から global に昇格すべきものを抽出して」といった相談で発火します。

---

> Release の zip は `audit-driven-feedback-vX.Y.Z.zip` / `assurance-audit-vX.Y.Z.zip` /
> `knowledge-audit-vX.Y.Z.zip` の **別ファイル**です（同じ Release に添付されますが、
> 1つのzipに全部入っているわけではありません）。使いたいスキルの zip だけ取得して展開すれば足ります。

## 背景記事

各スキルは Zenn 記事の方法論を実践手順化したものです。記事が思想、スキルが実践手順、
`evals/` が再現性確認、という3点セットで構成されています。

📝 **[AIエージェント時代の品質保証 ― 監査駆動フィードバック開発という考え方](https://zenn.dev/ichikawa_y/articles/audit-driven-feedback-development)** (`audit-driven-feedback` / `assurance-audit`)

📝 **[Claude Memoryに全部入れたら破綻した──知識にもCIではなくControllerが必要だった](https://zenn.dev/ichikawa_y/articles/knowledge-layer-audit)** (`knowledge-audit`)
...
- **release**: `vX.Y.Z` 形式のタグを push すると、CI が lint を通したうえで
  `audit-driven-feedback-vX.Y.Z.zip` / `assurance-audit-vX.Y.Z.zip` / `knowledge-audit-vX.Y.Z.zip`
  の3つをビルドし、GitHub Release に添付します。バージョンはタグで管理します（初回は `v1.0.0`）。
